...
function getFilteredProducts(mysqli $conn, string $filter = '', int $page = 1, int $recordsPerPage = 10): array|false
{
    ...
    $sqlCount = "SELECT COUNT(P.idProducto) as total FROM Productos P LEFT JOIN Proveedores Pr ON P.proveedorId = Pr.idProveedor LEFT JOIN Categorias C ON P.CategoriaId = C.idCategoria WHERE P.nombreProducto LIKE ? OR Pr.nombreProveedor LIKE ? OR C.nombreCategoria LIKE ?";
...
function getAllProviders(mysqli $conn, int $page = 1, int $recordsPerPage = 10): array|false
{
    ...
    $sqlData = "SELECT idProveedor, nombreProveedor, descripcionProveedor, direccionProveedor, telefono, Correo, infoAdicional FROM Proveedores ORDER BY idProveedor DESC LIMIT ? OFFSET ?";
    $stmtData = $conn->prepare($sqlData);
...
{
    $limit = max(1, $limit);
    $sql = "SELECT P.nombreProducto, SUM(DV.cantidad) AS total_vendido FROM detalle_venta DV JOIN productos P ON DV.productoId = P.idProducto GROUP BY P.idProducto, P.nombreProducto ORDER BY total_vendido DESC LIMIT ?";
...
function getSalesDataForChart(mysqli $conn, int $days): array
{
    ...
    $endDate = date('Y-m-d');
    $sql = "SELECT DATE(fechaVenta) AS fecha, SUM(totalVenta) AS total_ventas FROM ventas WHERE DATE(fechaVenta) BETWEEN ? AND ? GROUP BY DATE(fechaVenta)";
    $stmt = $conn->prepare($sql);
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            ...
            while ($row = $result->fetch_assoc()) {
                if (isset($salesData[$row['fecha']])) { $salesData[$row['fecha']] = $row['total_ventas']; }
            }
        }
        $stmt->close();
    }
...
function getProductsWithProviders(mysqli $conn, int $page = 1, int $recordsPerPage = 10): array|false
{
    ...
    $sql = "SELECT P.nombreProducto, P.cantidad, Pr.nombreProveedor, Pr.telefono, Pr.Correo FROM Productos P INNER JOIN Proveedores Pr ON P.proveedorId = Pr.idProveedor ORDER BY P.idProducto DESC LIMIT ? OFFSET ?";
    $stmt = $conn->prepare($sql);
    if(!$stmt) return false;
...
function getAllEntries(mysqli $conn, int $page = 1, int $recordsPerPage = 10): array|false
{
    ...
    $sql = "SELECT E.idEntrada, E.fechaEntrada, P.nombreProducto, E.cantidadComprada, E.precioCompraUnidad, Pr.nombreProveedor FROM EntradaProductos E LEFT JOIN Productos P ON E.productoId = P.idProducto LEFT JOIN Proveedores Pr ON E.proveedorId = Pr.idProveedor ORDER BY E.idEntrada DESC LIMIT ? OFFSET ?";
    $stmt = $conn->prepare($sql);
    if(!$stmt) return false;
    $stmt->bind_param('ii', $recordsPerPage, $offset);
    if(!$stmt->execute()) return false;
...
function getRecentSales(mysqli $conn, int $page = 1, int $recordsPerPage = 10): array|false
{
    ...
    $sql = "SELECT V.idVenta, V.fechaVenta, V.totalVenta, Vd.nombreCompleto FROM ventas V LEFT JOIN usuario Vd ON V.vendedorId = Vd.idUsuario ORDER BY V.idVenta DESC LIMIT ? OFFSET ?";
    $stmt = $conn->prepare($sql);
    if (!$stmt) { return false; }
    $stmt->bind_param('ii', $recordsPerPage, $offset);
    if (!$stmt->execute()) { $stmt->close(); return false; }
